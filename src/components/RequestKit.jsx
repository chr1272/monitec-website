...
// Safari on macOS enforces strict third-party cookie / iframe restrictions
// that block the Google Calendar scheduling iframe from loading at all.
// To avoid changing behavior for everyone else, we only swap in the
// button-based fallback for macOS users and keep the original iframe embed
...
export default function RequestKit() {
  ...
  const [useMacFallback, setUseMacFallback] = useState(false);

  useEffect(() => {
    setUseMacFallback(isMacOS());
  }, []);
  ...
  return (
    <section id="request-kit" className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
      <div className="overflow-hidden rounded-card bg-primary px-8 py-16 sm:px-16">
        ...
        {useMacFallback ? (
          // macOS/Safari fallback: a clean CTA card instead of the iframe,
          // which Safari blocks by default due to strict cookie controls.
          ...
          // Default: Google Calendar Appointment Scheduling embed.
